...
class _AddUserScreenState extends State<AddUserScreen> {
  ...
  Future<void> _handleSubmit() async {
    // Basic validation
    final invalidUser = _usersInput.any((user) =>
        user['firstName'] == '' ||
        user['workEmail'] == '' ||
        user['password'] == '');
    ...
    try {
      ...
      if (results != null) {
        ...
        if (successCount > 0) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Successfully added $successCount users!'), backgroundColor: Colors.green),
          );
          if (failCount == 0) {
            setState(() {
              _usersInput.clear();
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Failed to add $failCount users. Check details.'), backgroundColor: Colors.red),
            );
          }
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Failed to add users. Please check data.'), backgroundColor: Colors.red),
          );
        }
      }
    } catch (e) {
          ...
          children: [
            Expanded(
              child: OutlinedButton.icon(
                ...
                icon: const Icon(Icons.add, color: Color(0xFF20E19F)),
                label: const Text('Add Row', style: TextStyle(color: Color(0xFF20E19F))),
                style: OutlinedButton.styleFrom(
                  side: const BorderSide(color: Color(0xFF20E19F)),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _buildTextField('First Name *', user['firstName'], (v) => _updateField(uId, 'firstName', v)),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: _buildTextField('Last Name', user['lastName'], (v) => _updateField(uId, 'lastName', v)),
                      ),
                    ],
                  ),
                  ...
                  Row(
                    children: [
                      Expanded(
                        flex: 2,
                        child: _buildTextField('Work Email *', user['workEmail'], (v) => _updateField(uId, 'workEmail', v), keyboard: TextInputType.emailAddress),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        flex: 1,
                            ...
                            Container(
                              ...
                              child: DropdownButtonHideUnderline(
                                child: DropdownButton<String>(
                                  ...
                                  onChanged: (v) => _updateField(uId, 'role', v!),
                                ),
                              ),
                            ),
                          ],
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _buildTextField('Temporary Password *', user['password'], (v) => _updateField(uId, 'password', v), obscureText: false),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: _buildTextField('Mobile No.', user['mobileNumber'], (v) => _updateField(uId, 'mobileNumber', v), keyboard: TextInputType.phone),
                      ),
                    ],
                  ),
                  const SizedBox(height: 12),
                  Row(
                    children: [
                      Expanded(
                        child: _buildTextField('Designation', user['designation'], (v) => _updateField(uId, 'designation', v)),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: _buildTextField('Department', user['department'], (v) => _updateField(uId, 'department', v)),
                      ),
                    ],
                  ),
                ],
  ...
  Widget _buildTextField(String label, String value, Function(String) onChanged, {TextInputType keyboard = TextInputType.text, bool obscureText = false}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
