...
const calculateReminderTime = (dueDate, timeValue, timeUnit, triggerType) => {
    ...
    // Ensure we handle date-only strings by setting them to end of day if no time is present
    let date;
    if (typeof dueDate === 'string' && dueDate.length === 10) {
        // YYYY-MM-DD -> treat as 23:59:59 in local time
        date = new Date(`${dueDate}T23:59:59`);
    } else {
    ...
    const result = triggerType === 'before' 
        ? new Date(date.getTime() - ms)
        : new Date(date.getTime() + ms);

    console.log(`[calculateReminderTime] Due: ${date.toISOString()}, Trigger: ${value} ${timeUnit} ${triggerType}, Reminder: ${result.toISOString()}`);
    return result;
};
...
export const createDelegation = async (req, reply) => {
    ...
    if (!taskTitle?.trim()) return reply.status(400).send({ success: false, message: 'Task Title is required' });
    if (!description?.trim()) return reply.status(400).send({ success: false, message: 'Task Description is required' });
    if (!doerId || (Array.isArray(doerId) && doerId.length === 0)) return reply.status(400).send({ success: false, message: 'Assignee is required' });
...
export const updateDelegation = async (req, reply) => {
    ...
    try {
        ...
        // Notify everyone in loop (In-app & External)
        const loopIds = Array.isArray(updatedDelegation.inLoopIds) ? updatedDelegation.inLoopIds : (typeof updatedDelegation.inLoopIds === 'string' ? JSON.parse(updatedDelegation.inLoopIds) : []);
        const loopEventType = isStatusChanged 
            ? (updates.status === 'Completed' ? 'loopTaskComplete' : 'loopTaskInProgress') 
            : 'loopTaskEdit';
        
        for (const loopId of loopIds) {
            if (loopId === changedBy) continue;
            ...
            await notifyUser(loopId, loopEventType, {
                title: title + ' (In Loop)',
                message: `${message}\nTask: ${existingDelegation.taskTitle}\nUpdated by: ${updaterName}`,
                html: `
                    <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
                        <h2 style="color: #6366f1;">In Loop: ${title}</h2>
                        <p>${message}</p>
                        <p><strong>Task:</strong> ${existingDelegation.taskTitle}</p>
                        <p><strong>Updated by:</strong> ${updaterName}</p>
                        <hr style="border: none; border-top: 1px solid #eee; margin: 20px 0;">
...
export const addRemark = async (req, reply) => {
    ...
    try {
        ...
        if (delegation) {
            ...
            // Notify everyone in loop (In-app & External)
            const loopIds = Array.isArray(delegation.inLoopIds) ? delegation.inLoopIds : (typeof delegation.inLoopIds === 'string' ? JSON.parse(delegation.inLoopIds) : []);
            for (const loopId of loopIds) {
                if (loopId === userId) continue;

                await createNotification(
                    loopId,
                    'New Remark (In Loop)',
                    `A new remark was added to task [${delegation.taskTitle}] by ${commenterName}`,
                    'remark',
                    id
                );

                await notifyUser(loopId, 'loopTaskComment', {
                    title: 'New Remark Added (In Loop)',
                    message: `A new remark has been added to task "${delegation.taskTitle}" by ${commenterName}: ${remark}`,
                    html: `
                        <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
                            <h2 style="color: #6366f1;">In Loop: New Remark Added</h2>
                            <p><strong>Task:</strong> ${delegation.taskTitle}</p>
                            <p><strong>Remark:</strong> ${remark}</p>
                            <p><strong>Added by:</strong> ${commenterName}</p>
                            <hr style="border: none; border-top: 1px solid #eee; margin: 20px 0;">
