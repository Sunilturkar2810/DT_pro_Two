...
import { 
    Mail, Smartphone, Save, Loader2, Info, Plus, Trash2, 
...
const NotificationTemplates = () => {
    ...
    const events = {
        ...
        reminder: 'Custom',
        // In Loop events
        loopNewTask: 'In Loop: New Task',
        loopTaskEdit: 'In Loop: Task Edited',
        loopTaskComment: 'In Loop: Task Comment',
        loopTaskInProgress: 'In Loop: Task In-Progress',
        loopTaskComplete: 'In Loop: Task Complete',
        loopTaskReOpen: 'In Loop: Task Re-Open'
    };
    ...
    const variables = [
        ...
        { key: '{taskList}', label: 'Task List', description: 'Summary list of pending tasks' },
        { key: '{voiceNoteUrl}', label: 'Voice Note URL', description: 'Link to task voice note' },
        { key: '{referenceDocs}', label: 'Ref Docs URL', description: 'Link to reference documents' },
        { key: '{tags}', label: 'Tags', description: 'Task tags' },
        { key: '{checklistItems}', label: 'Checklist Items', description: 'List of checklist items' },
        { key: '{evidenceRequired}', label: 'Evidence Required', description: 'Whether evidence is required' },
        { key: '{evidenceUrl}', label: 'Evidence URL', description: 'Link to submitted evidence' },
        { key: '{revisionCount}', label: 'Revision Count', description: 'Number of times task was revised' },
        { key: '{inLoopIds}', label: 'In Loop', description: 'List of users in loop' },
        { key: '{fromDate}', label: 'From Date', description: 'Task start date' },
        { key: '{frequency}', label: 'Frequency', description: 'Task repeat frequency' }
    ];
                    ...
                    {loading ? (
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-100 dark:border-slate-800 shadow-sm space-y-6">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                <div className="lg:col-cols-2 space-y-6 lg:col-span-2">
                                    {activeChannel === 'email' && (
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                                            {activeChannel === 'email' ? 'Email Body (HTML supported)' : 'WhatsApp Message'}
                                        </label>
                                        <textarea
                                            id="template-body"
                                            value={template.body}
                                            onChange={(e) => setTemplate(prev => ({ ...prev, body: e.target.value }))}
                                            placeholder={`Enter your ${activeChannel === 'email' ? 'email content' : 'WhatsApp message'} here...`}
                                            className="w-full h-80 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-3xl px-6 py-5 text-sm font-medium text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all placeholder:text-slate-400 resize-none font-mono"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-2 p-4 bg-emerald-50 dark:bg-emerald-500/10 rounded-2xl border border-emerald-100 dark:border-emerald-500/20">
                                        ...
                                        <p className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 leading-tight">
                                            Click on a variable to insert it into your {activeChannel === 'whatsapp' ? 'message' : 'email body'} at the current cursor position.
                                        </p>
                                    </div>
