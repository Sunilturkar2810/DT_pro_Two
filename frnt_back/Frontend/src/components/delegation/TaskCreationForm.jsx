...
const TaskCreationForm = ({ isOpen, onClose, onSuccess, groupId, initialData, parentId }) => {
        ...
        try {
            ...
            const submissionData = {
                ...
                reminders: (reminders || []).map(r => ({
                    type: r.type || 'email',
                    timeValue: parseInt(r.timeValue || '0'),
                    timeUnit: r.timeUnit || 'minutes',
                    triggerType: (r.triggerType || 'before').toLowerCase()
                }))
            };
    ...
    const filteredUsers = (users || []).filter(u => {
        ...
        // and check if status is active (usually you'd only want active users)
                                ...
                                {filteredUsers.map(u => (
                                    <div key={u.userId} onClick={() => initialData?.id ? setFormData(prev => ({ ...prev, doerId: [u.userId] })) : toggleUserSelect(u.userId, 'doerId')} className={`p-3.5 hover:bg-emerald-50/50 rounded-2xl cursor-pointer flex items-center gap-4 transition-all border-2 ${formData.doerId.includes(u.userId) ? 'bg-emerald-50 border-emerald-100 shadow-sm' : 'border-transparent'}`}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="text-[11px] text-slate-400 font-bold truncate tracking-tight">{u.email || u.workEmail}</div>
                                        </div>
                                    </div>
                                ))}
                                        ...
                                        {filteredLoopUsers.map(u => (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="text-[12px] text-slate-400 font-bold truncate tracking-widest uppercase opacity-70">{u.email || u.workEmail}</div>
                                                </div>
                                                {!formData.inLoopIds.includes(u.userId) && (
