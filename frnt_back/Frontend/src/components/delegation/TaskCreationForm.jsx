...
const TaskCreationForm = ({ isOpen, onClose, onSuccess, groupId, initialData, parentId }) => {
    ...
    const [reminders, setReminders] = useState([{
        id: 1,
        type: 'both',  // both, WhatsApp, Email
        timeValue: '10',
        timeUnit: 'minutes',
        triggerType: 'before' // before, after
    }]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (formData.inLoopIds.length === 0) throw new Error('In Loop is required');
            if (!formData.priority) throw new Error('Priority is required');

            let finalVoiceNoteUrl = initialData?.voiceNoteUrl || null;
            let finalReferenceDocs = [];

            // 1. Upload Voice Note (only if new recording exists)
            if (audioBlob) {
            ...
            // 3. Process existing links and newly uploaded docs
            if (links.length > 0) {
            ...
            const submissionData = {
                ...
                referenceDocs: finalReferenceDocs.length > 0 ? [...new Set(finalReferenceDocs)].join(',') : null,
                ...
                reminders: reminders.map(r => ({
                    type: r.type,
                    timeValue: parseInt(r.timeValue),
                    timeUnit: r.timeUnit,
                    triggerType: r.triggerType.toLowerCase()
                }))
            };
            ...
            try {
                if (isEdit) {
                    await delegationService.updateDelegation(initialData.id, submissionData);
            ...
            <div className="relative w-full max-w-[680px] sm:mt-0 h-full sm:h-auto max-h-screen sm:max-h-[92vh] bg-white sm:rounded-2xl shadow-[0_20px_70px_rgba(0,0,0,0.15)]  border border-slate-200/60 animate-in zoom-in-95 duration-300 flex flex-col overflow-visible">
                ...
                <div className="flex items-center justify-between px-4 py-2 border-b border-slate-100 bg-[#e6f9f1] relative sm:rounded-t-2xl">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xs font-bold text-slate-800 leading-tight">{initialData?.id ? 'Edit Task' : 'Assign New Task'}</h2>
                            <p className="text-[8px] text-emerald-600 font-bold uppercase tracking-widest opacity-80">{initialData?.id ? 'Modify Delegation' : 'New Delegation'}</p>
                        </div>
                    </div>
                    ...
                    >
                        <div className="flex flex-col gap-4">
                            <div className="p-1 border-b border-slate-50">
                                ...
                                {filteredUsers.map(u => (
                                    <div key={u.userId} onClick={() => initialData?.id ? setFormData(prev => ({ ...prev, doerId: [u.userId] })) : toggleUserSelect(u.userId, 'doerId')} className={`p-3.5 hover:bg-emerald-50/50 rounded-2xl cursor-pointer flex items-center gap-4 transition-all border-2 ${formData.doerId.includes(u.userId) ? 'bg-emerald-50 border-emerald-100 shadow-sm' : 'border-transparent'}`}>
                                    ...
                                    <div className="flex flex-col gap-7">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-1.5 bg-slate-100/50 p-1.5 rounded-2xl border border-slate-100">
                                                {['Both', 'WhatsApp', 'Email'].map(m => (
                                                    <button key={m} onClick={() => updateReminder(reminder.id, 'type', m.toLowerCase())} className={`px-3 py-1.5 rounded-lg text-[10px] font-black transition-all uppercase tracking-wide ${reminder.type === m.toLowerCase() ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{m}</button>
                                                ))}
                                            </div>
                                        </div>
