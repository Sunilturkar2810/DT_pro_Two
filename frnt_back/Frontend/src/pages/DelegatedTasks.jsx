...
const DelegatedTasks = () => {
    ...
    // Client-side filtering: only tasks where current user is assigner
    const filteredTasks = tasks.filter(t => {
        if (t.assignerId !== currentUserId) return false;
    ...
    const getStatusCount = (status) => tasks.filter(t => t.assignerId === currentUserId && (status === 'All' || t.status === status)).length;
    ...
    const handleExport = () => {
        ...
        exportToExcel(exportData, `Delegated_Tasks_${new Date().toISOString().split('T')[0]}`, 'Delegated Tasks');
        ...
        <div className="min-h-screen bg-[#e6f9f1] p-6 lg:p-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-wrap items-end gap-3 mb-8">
                ...
                <button 
                    ...
                    className="flex items-center gap-2 px-5 h-11 bg-[#00d094] hover:bg-[#00ba84] text-white rounded-lg font-bold text-sm transition-all active:scale-95 shadow-sm"
                >
                ...
                {dateRange === 'Custom' && (
                    <>
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="relative border border-[#00d094] rounded-lg h-11 flex items-center px-2.5 gap-1.5 bg-white min-w-[130px]">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <input 
                                        ...
                                        className="bg-transparent border-none outline-none text-[11px] font-bold text-slate-700 w-full dark:[color-scheme:light]"
                                    />
                                </div>
                            </div>
                        </div>
                ...
                <div className="relative flex-1 max-w-sm flex flex-col gap-1">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 invisible">Search</span>
                ...
                <button
                    ...
                    className="h-11 w-11 flex items-center justify-center bg-[#00d094] text-white rounded-lg hover:bg-[#00ba84] transition-all shadow-sm"
                    title="Clear Filters"
                >
                ...
                <button 
                    ...
                    className="flex items-center gap-2 px-4 h-11 bg-[#00d094] hover:bg-[#00ba84] text-white rounded-lg font-bold text-sm transition-all shadow-sm"
                >
                    <FileUp size={18} />
                    Export
                </button>
                ...
                {/* View Mode */}
                <div className="flex bg-white rounded-lg p-1 border border-slate-200 h-11 items-center">
                    {[
                        { mode: 'List', icon: List },
                    ...
                    {[
                        ...
                        >
                            ...
                            {statusFilter === tab.key && (
                                <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#00d094] rounded-t-full" />
                            )}
                        </button>
                    ))}
                </div>
            ...
            <div className="max-w-7xl mx-auto space-y-3 pb-20">
                {loading ? (
                    ...
                    filteredTasks.map((task) => (
                        <div 
                            key={task.id}
                            className={`bg-white rounded-xl border border-white shadow-sm transition-all duration-300 ${expandedTasks.has(task.id) ? 'ring-2 ring-[#00d094]/30' : 'hover:shadow-md'}`}
                        >
                            <div 
                            ...
                            >
                                ...
                                <div className="w-10 h-10 rounded-full bg-[#00d094]/10 text-[#00d094] flex items-center justify-center font-bold relative shrink-0">
                                    <div className="absolute inset-0 rounded-full border-2 border-sky-400 opacity-50" />
                                    {getInitials(task.doerFirstName, task.doerLastName)}
                                </div>
                                <div className="flex items-center gap-4 flex-1 min-w-0">
                                    <div className="flex items-center gap-2 min-w-0">
                                        <span className="text-sm font-bold text-slate-500 whitespace-nowrap hidden sm:block">{task.doerFirstName} {task.doerLastName}</span>
