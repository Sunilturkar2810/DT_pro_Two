...
const FilterChip = ({ label, onRemove }) => (
    <div className="flex items-center gap-1.5 px-3 py-1 bg-white border border-[#00d094]/40 text-[#00d094] rounded-full text-[11px] font-bold">
        {label}
...
const AllTasks = () => {
    ...
    const [isHighlighted, setIsHighlighted] = useState(false); // glows when coming from dashboard
    ...
    const [exportTaskTypes, setExportTaskTypes] = useState([]); // 'repetitive', 'one-time'
    ...
    // On mount: read URL params set by Dashboard drilldown
    useEffect(() => {
        ...
        if (paramHL === 'true') {
            ...
            // Fade the highlight glow after 3 seconds
    ...
    // when export modal opens, initialize its filters
    useEffect(() => {
        if (showExportModal) {
    ...
    // helper for checkbox multi-select
    ...
    // filter function used by export modal
    const getTasksForExport = (opts) => {
        const {
            range = 'All Time', start = '', end = '',
            aTo = [], aBy = [], cat = 'All', types = []
        } = opts;
    ...
    const activeFilterCount = [
        ...
        assignedTo !== 'All', assignedBy !== 'All', tagFilter !== 'All'
    ].filter(Boolean).length;

    // Summary stat cards at top
    const quickStats = [
        { label: 'Total', value: tasks.length, dot: 'bg-slate-400', bg: 'bg-white', color: 'text-slate-700' },
        { label: 'Overdue', value: getStatusCount('Overdue'), dot: 'bg-red-500', bg: 'bg-red-50', color: 'text-red-600' },
        { label: 'Pending', value: getStatusCount('Pending'), dot: 'border-2 border-slate-400', bg: 'bg-slate-50', color: 'text-slate-600' },
        { label: 'In Progress', value: getStatusCount('In Progress'), dot: 'bg-orange-500', bg: 'bg-orange-50', color: 'text-orange-600' },
        { label: 'Completed', value: getStatusCount('Completed'), dot: 'bg-emerald-500', bg: 'bg-emerald-50', color: 'text-emerald-600' },
    ];

    return (
        <div className="min-h-screen bg-[#e6f9f1] p-6 lg:p-8 animate-in fade-in duration-500">

            {/* ── Page Title ── */}
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-[#00d094] rounded-xl flex items-center justify-center shadow-lg shadow-[#00d094]/30">
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-5 h-10 bg-[#00d094] hover:bg-[#00ba84] text-white rounded-lg font-bold text-sm transition-all active:scale-95 shadow-sm"
                >
            ...
            <div className="flex flex-wrap items-end gap-3 mb-8">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <div className="relative min-w-[140px]">
                        <select
                            ...
                            className="w-full h-11 bg-white border border-[#00d094] rounded-lg pl-3 pr-8 text-sm font-bold text-slate-700 outline-none appearance-none cursor-pointer shadow-sm"
                        >
                ...
                {dateRange === 'Custom' && (
                    <>
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="border border-[#00d094] rounded-lg h-11 flex items-center px-2.5 gap-1.5 bg-white min-w-[130px]">
                                <CalendarIcon size={14} className="text-[#00d094] shrink-0" />
                ...
                <div className="relative flex flex-col gap-1" ref={filterPanelRef}>
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-5 h-11 rounded-lg font-bold text-sm transition-all shadow-sm ${showFilters ? 'bg-slate-800 text-white' : 'bg-[#00d094] hover:bg-[#00ba84] text-white'}`}
                    >
                        <Filter size={18} />
                        Filter
                        {activeFilterCount > 0 && (
                            <span className="bg-white text-[#00d094] text-[10px] font-black rounded-full w-5 h-5 flex items-center justify-center">
                                {activeFilterCount}
                            </span>
                        )}
                    </button>
                    ...
                    {showFilters && (
                        <div className="absolute top-[calc(100%+8px)] left-0 z-50 bg-white border border-slate-200 rounded-xl shadow-2xl p-4 flex flex-col gap-4 min-w-[240px] animate-in slide-in-from-top-2 duration-200">
                            <div className="flex items-center justify-between">
                                ...
                                <button onClick={handleClearFilters} className="text-[10px] font-bold text-[#00d094] hover:underline">Clear All</button>
                            </div>

                            <div className="flex flex-col gap-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase">Assigned To</label>
                                <select value={assignedTo} onChange={e => setAssignedTo(e.target.value)} className="bg-slate-50 border border-slate-100 p-2 rounded-lg text-xs font-bold outline-none">
                ...
                <div className="relative flex-1 max-w-sm flex flex-col gap-1">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full h-11 pl-10 pr-4 bg-white border border-slate-200 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-emerald-500/20"
                        />
                    </div>
                </div>
                ...
                <button onClick={handleClearFilters} title="Clear Filters"
                    className="h-11 w-11 flex items-center justify-center bg-[#00d094] text-white rounded-lg hover:bg-[#00ba84] transition-all shadow-sm self-end">
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-4 h-11 rounded-lg font-bold text-sm transition-all shadow-sm self-end
                ...
                <div className="flex bg-white rounded-lg p-1 border border-slate-200 h-11 items-center self-end">
                    {[
                        ...
                        <button key={mode} onClick={() => setViewMode(mode)}
                            className={`p-2 rounded-md transition-all ${viewMode === mode ? 'bg-[#00d094] text-white shadow-sm' : 'text-slate-400 hover:bg-slate-50'}`}>
                            <Icon size={18} />
                        </button>
                    ))}
                </div>
            ...
            {showExportModal && (
                <div onClick={() => setShowExportModal(false)} className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                    <div onClick={e => e.stopPropagation()} className="bg-white rounded-lg w-full max-w-md p-6 relative max-h-[90vh] overflow-y-auto">
                        <button
                        ...
                        <h2 className="text-lg font-bold mb-4">Export Tasks</h2>
                        <div className="space-y-4">
                            <div className="flex flex-col gap-1">
                                ...
                                <select
                                    ...
                                    className="bg-slate-50 border border-slate-100 p-2 rounded-lg text-xs font-bold outline-none"
                                >
                                    <option value="All Time">All Time</option>
                                    <option value="Today">Today</option>
                                    <option value="Yesterday">Yesterday</option>
                            ...
                            {exportDateRange === 'Custom' && (
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        className="border border-slate-100 p-2 rounded-lg text-xs"
                                    />
                                    <input
                                        type="date"
                            ...
                            <div className="flex flex-col gap-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase">Assigned To</label>
                                <div className="bg-slate-50 border border-slate-100 p-2 rounded-lg text-xs font-bold max-h-32 overflow-y-auto">
                                    <div className="flex items-center gap-2 mb-1">
                                        <input
                                            type="checkbox"
                                            checked={exportAssignedTo.length === users.length}
                                            onChange={() => toggleAll(exportAssignedTo, setExportAssignedTo)}
                                        />
                                    ...
                                    {users.map(u => {
                                        ...
                                        return (
                                            <div key={id} className="flex items-center gap-2">
                                                <input
                                                    type="checkbox"
                                                    checked={exportAssignedTo.includes(id)}
                                                    onChange={() => toggleSelection(id, exportAssignedTo, setExportAssignedTo)}
                                                />
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <div className="bg-slate-50 border border-slate-100 p-2 rounded-lg text-xs font-bold max-h-32 overflow-y-auto">
                                    <div className="flex items-center gap-2 mb-1">
                                        <input
                                            type="checkbox"
                                            checked={exportAssignedBy.length === users.length}
                                            onChange={() => toggleAll(exportAssignedBy, setExportAssignedBy)}
                                        />
                                        <span>Select All</span>
                                    </div>
                                    {users.map(u => {
                                        ...
                                        return (
                                            <div key={id} className="flex items-center gap-2">
                                                <input
                                                    type="checkbox"
                                                    checked={exportAssignedBy.includes(id)}
                                                    onChange={() => toggleSelection(id, exportAssignedBy, setExportAssignedBy)}
                                                />
                                                <span>{u.firstName} {u.lastName}</span>
                                            </div>
                                        );
                            ...
                            {/* category/type pick */}
                            <div className="flex flex-col gap-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase">Task Type</label>
                                <div className="bg-slate-50 border border-slate-100 p-2 rounded-lg text-xs font-bold max-h-32 overflow-y-auto">
                                    <div className="flex items-center gap-2 mb-1">
                                        <input
                                            type="checkbox"
                                            checked={exportTaskTypes.includes('repetitive') && exportTaskTypes.includes('one-time')}
                                            onChange={() => {
                                                if (exportTaskTypes.length === 2) setExportTaskTypes([]);
                                                else setExportTaskTypes(['repetitive','one-time']);
                                            }}
                                        />
                                        <span>Select All</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <input
                                            type="checkbox"
                                            checked={exportTaskTypes.includes('repetitive')}
                                            onChange={() => {
                                                const idx = exportTaskTypes.indexOf('repetitive');
                                                if (idx > -1) setExportTaskTypes(exportTaskTypes.filter(x => x !== 'repetitive'));
                                                else setExportTaskTypes([...exportTaskTypes,'repetitive']);
                                            }}
                                        />
                                    ...
                                    <div className="flex items-center gap-2">
                                        <input
                                            type="checkbox"
                                            checked={exportTaskTypes.includes('one-time')}
                                            onChange={() => {
                                                const idx = exportTaskTypes.indexOf('one-time');
                                                if (idx > -1) setExportTaskTypes(exportTaskTypes.filter(x => x !== 'one-time'));
                                                else setExportTaskTypes([...exportTaskTypes,'one-time']);
                                            }}
                                        />
                        ...
                        <div className="flex justify-end mt-6 gap-2">
                            <button
                                onClick={() => {
                                    const toExport = getTasksForExport({
                                        range: exportDateRange,
                                        start: exportCustomStart,
                                        end: exportCustomEnd,
                                        aTo: exportAssignedTo,
                                        aBy: exportAssignedBy,
                                        cat: exportCategory,
                            ...
                            >
                                {`Export Tasks${getTasksForExport({
                                    range: exportDateRange,
                                    start: exportCustomStart,
                                    end: exportCustomEnd,
                                    aTo: exportAssignedTo,
                            ...
                            <button
                                onClick={() => setShowExportModal(false)}
                                className="px-5 py-2 bg-slate-200 text-slate-600 rounded-lg font-bold hover:bg-slate-300 transition-all"
                            >
                                Cancel
                            </button>
                        </div>
                ...
                <div className="flex gap-6 overflow-x-auto">
                    {[
                        ...
                        <button key={tab.key} onClick={() => setStatusFilter(tab.key)}
                            ...
                            {statusFilter === tab.key && <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#00d094] rounded-t-full" />}
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto space-y-3 pb-20">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-4">
                        <div className="w-12 h-12 border-4 border-[#00d094] border-t-transparent rounded-full animate-spin" />
                    ...
                    <div className="bg-white/40 border-2 border-dashed border-white/60 rounded-3xl p-20 flex flex-col items-center justify-center text-center">
                        ...
                        {activeFilterCount > 0 && (
                            <button onClick={handleClearFilters} className="mt-4 px-4 py-2 bg-[#00d094] text-white rounded-lg text-sm font-bold hover:bg-[#00ba84] transition-all">
                                Clear Filters
                            </button>
                        )}
                    </div>
                    ...
                    filteredTasks.map((task) => (
                        <div key={task.id}
                            className={`bg-white rounded-xl border border-white shadow-sm transition-all duration-300 
                                ${expandedTasks.has(task.id) ? 'ring-2 ring-[#00d094]/30' : 'hover:shadow-md'}
                                ${isHighlighted ? 'ring-2 ring-[#00d094] shadow-[0_0_20px_rgba(0,208,148,0.3)]' : ''}
                            `}>
                            {/* Dashboard highlight banner */}
                            {isHighlighted && (
                                <div className="flex items-center gap-2 px-4 py-1.5 bg-[#00d094]/10 rounded-t-xl border-b border-[#00d094]/20">
                                    <div className="w-2 h-2 bg-[#00d094] rounded-full animate-ping" />
                                    <span className="text-[10px] font-black text-[#00d094] uppercase tracking-widest">From Dashboard Filter</span>
                                </div>
                            )}

                            <div className="p-4 flex items-center gap-4 cursor-pointer"
                                onClick={() => toggleTaskExpansion(task.id)}>
                                <input type="checkbox" className="w-5 h-5 rounded border-slate-300 accent-[#00d094]" onClick={e => e.stopPropagation()} />

                                {/* Doer avatar */}
                                <div className="w-10 h-10 rounded-full bg-[#00d094]/10 text-[#00d094] flex items-center justify-center font-bold relative shrink-0"
                                    title={`Assigned to ${task.doerFirstName || ''} ${task.doerLastName || ''}`}>
                                    <div className="absolute inset-0 rounded-full border-2 border-[#00d094]/40" />
                                ...
                                <div className="flex items-center gap-4 flex-1 min-w-0">
                                    <div className="flex flex-col min-w-0">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex items-center gap-1.5 scale-75 origin-left shrink-0">
                                                {task.voiceNoteUrl && <Mic size={14} className="text-emerald-500" strokeWidth={3} />}
                                    ...
                                    <div className="ml-auto flex items-center gap-3 shrink-0">
                                        ...
                                        <button
                                            ...
                                            className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-[#00d094] transition-all"
                                            title="View Details"
                                        >
                                ...
                                <div className="px-6 pb-5 pt-2 border-t border-slate-50 animate-in slide-in-from-top-2 duration-300 space-y-3">
                                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs font-bold text-slate-400 pt-2">
                                        <div className="flex items-center gap-1.5 text-slate-500">
                                            ...
                                            Due: {task.dueDate
                                                ? new Date(task.dueDate).toLocaleString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true })
                                                : 'No date set'}
                                        </div>
                                        <div className="flex items-center gap-1.5">
                                            <User size={14} className="text-purple-400" />
                                            <span>By: <span className="text-slate-600">{task.assignerFirstName} {task.assignerLastName}</span></span>
                                        </div>
                                        <div className="flex items-center gap-1.5">
                                            <User size={14} className="text-[#00d094]" />
                                            <span>To: <span className="text-slate-600">{task.doerFirstName} {task.doerLastName}</span></span>
                                        </div>
                                        {task.category && (
                                            <div className="flex items-center gap-1.5">
                                    ...
                                    {/* Description */}
                                    {task.description && (
                                        <p className="text-xs text-slate-500 font-medium leading-relaxed line-clamp-2 border-l-2 border-[#00d094]/30 pl-3">
                                            {task.description}
                                        </p>
                                    )}
                                    ...
                                    <div className="pt-1">
                                        <button
                                            onClick={(e) => { e.stopPropagation(); setSelectedTaskId(task.id); setShowDetails(true); }}
                                            className="flex items-center gap-1.5 px-4 py-2 bg-[#00d094]/10 hover:bg-[#00d094]/20 text-[#00d094] rounded-lg text-[11px] font-black uppercase tracking-widest transition-all"
                                        >
