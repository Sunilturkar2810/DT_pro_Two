...
const STATUS_MAP = {
    ...
    Completed:    { dot: 'bg-emerald-500', text: 'text-emerald-600', bg: 'bg-emerald-50',border: 'border-emerald-200' },
};
...
const DeletedTasks = () => {
    ...
    const isAdmin      = ['ADMIN', 'ADMIN', 'ADMIN', 'SUPERADMIN'].includes(currentUser?.role);
                        ...
                        return (
                            <div key={task.id} className="bg-white rounded-xl border border-slate-100 p-4 shadow-sm hover:shadow-md transition-all flex items-start gap-4">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 border-slate-200 hover:border-emerald-400 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 font-bold text-sm transition-all active:scale-95 disabled:opacity-50 shrink-0"
                                >
                                    {restoringId === task.id ? (
