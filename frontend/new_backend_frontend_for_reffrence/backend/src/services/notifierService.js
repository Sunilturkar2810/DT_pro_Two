...
export const notifyUser = async (userId, eventType, data) => {
    try {
        ...
        if (eventType === 'reminder' || eventType === 'dailyPendingReminders') {
            ...
            const isEmailEnabled = eventType === 'reminder' ? currentPrefs.emailReminders : currentPrefs.emailNotifications;
            const isWhatsappEnabled = eventType === 'reminder' ? currentPrefs.whatsappReminders : currentPrefs.whatsappNotifications;
