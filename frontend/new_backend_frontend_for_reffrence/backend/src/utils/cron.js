...
const sendDailyTaskReports = async () => {
    const now = new Date();
    // Format to HH:mm in Asia/Kolkata (or system local, but preferences use HH:mm)
    const currentTime = now.toLocaleTimeString('en-GB', { 
        hour: '2-digit', 
        minute: '2-digit',
        hour12: false,
        timeZone: 'Asia/Kolkata' // Defaulting to system/app timezone
    });
    ...
    try {
        ...
        for (const pref of prefs) {
            ...
            const taskList = pendingTasks.map(t => `- ${t.taskTitle}${t.dueDate ? ` (Due: ${t.dueDate})` : ''}`).join('\n');
            ...
            console.log(`Sent daily task report to user ${user.userId}`);
        }
    } catch (error) {
        console.error('Error in sendDailyTaskReports:', error);
    }
};
