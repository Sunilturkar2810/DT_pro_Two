...
const TaskCreationForm = ({ isOpen, onClose, onSuccess, groupId, initialData, parentId }) => {
    ...
    const [reminders, setReminders] = useState([{
        id: 1,
        medium: 'Mobile App',  // Mobile App, WhatsApp, Email
        timingValue: '10',
        timingUnit: 'minutes',
        ...
        if (isOpen) {
            ...
            if (initialData) {
                ...
                if (initialData.checklistItems) {
                    setChecklistItems(initialData.checklistItems.map((item, idx) => ({
                        id: Date.now() + idx,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!formData.taskTitle) throw new Error('Task Title is required');
            if (formData.doerId.length === 0) throw new Error('Assignee is required');

            let finalVoiceNoteUrl = null;
            let finalReferenceDocs = [];

            // 1. Upload Voice Note
            if (audioBlob) {
                const voiceFile = new File([audioBlob], `voice-note-${Date.now()}.webm`, { type: 'audio/webm' });
                try {
                    const res = await delegationService.uploadFile(voiceFile, 'voice-notes');
                    if (res?.url) finalVoiceNoteUrl = res.url;
                } catch (e) {
            ...
            if (attachments.length > 0) {
                for (const file of attachments) {
                    try {
                        const res = await delegationService.uploadFile(file, 'documents');
                        if (res?.url) finalReferenceDocs.push(res.url);
                    } catch (e) {
            ...
            const submissionData = {
                ...
                referenceDocs: finalReferenceDocs.length > 0 ? finalReferenceDocs.join(',') : null,
                ...
                parentId: parentId // Pass the parent ID for subtasks
            };

            console.log('--- API Task Submission Payload ---');
            console.log(JSON.stringify(submissionData, null, 2));
            console.log('-----------------------------------');

            try {
                const response = await delegationService.createDelegation(submissionData);
    ...
    const filteredLoopUsers = users.filter(u =>
        (u.firstName + ' ' + u.lastName).toLowerCase().includes(loopSearch.toLowerCase()) ||
        (u.email || '').toLowerCase().includes(loopSearch.toLowerCase())
    );
            ...
            <div className="relative w-full max-w-[680px] sm:mt-0 h-full sm:h-auto max-h-screen sm:max-h-[92vh] bg-white sm:rounded-2xl shadow-[0_20px_70px_rgba(0,0,0,0.15)]  border border-slate-200/60 animate-in zoom-in-95 duration-300 flex flex-col overflow-visible">
                ...
                <div className="flex items-center justify-between px-4 py-2 border-b border-slate-100 bg-[#e6f9f1] relative sm:rounded-t-2xl">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xs font-bold text-slate-800 leading-tight">Assign New Task</h2>
                            <p className="text-[8px] text-emerald-600 font-bold uppercase tracking-widest opacity-80">New Delegation</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col min-h-0 bg-white">
                        ...
                        {tags.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-4">
                                {tags.map((tag, i) => (
                                    <div
                                        key={i}
                                        className="flex items-center gap-2 px-3 py-1.5 rounded-xl border-2 uppercase text-[10px] font-black tracking-widest shadow-sm transition-all"
                                        style={{ backgroundColor: `${tag.color}08`, borderColor: `${tag.color}25`, color: tag.color }}
                                    >
                                        <Tag size={10} strokeWidth={3} className="shrink-0" />
                                        <span>{tag.text}</span>
                                        <button onClick={() => setTags(prev => prev.filter((_, idx) => idx !== i))} className="w-4 h-4 rounded-full flex items-center justify-center hover:bg-black/5 transition-all -mr-1">
                    ...
                    <div className="p-3 flex flex-wrap items-center gap-1.5 border-b border-slate-50 relative bg-emerald-50/5">
                        ...
                        <div className="relative">
                            <button onClick={() => toggleModal('users')} className={`flex items-center justify-center gap-1.5 px-2.5 py-1 rounded-lg border-2 text-[8px] font-bold transition-all shadow-sm uppercase tracking-wide ${formData.doerId.length > 0 ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : 'border-slate-100 bg-white text-slate-400 hover:border-emerald-500/30'}`}>
                        ...
                        <div className="relative group">
                            <button onClick={() => toggleModal('date')} className={`flex items-center justify-center gap-1.5 px-2.5 py-1 rounded-lg border-2 text-[8px] font-bold transition-all shadow-sm uppercase tracking-wide ${formData.dueDate ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : 'border-slate-100 bg-white text-slate-400 hover:border-emerald-500/30'}`}>
                                <CalendarIcon size={14} className={formData.dueDate ? "text-emerald-500" : "text-slate-300"} />
                                {formData.dueDate ? formData.dueDate.toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : 'Due Date'}
                            </button>
                        </div>
                        ...
                        {/* 4. Category */}
                        <div className="relative">
                            <button onClick={() => toggleModal('category')} className={`flex items-center justify-center gap-1.5 px-2.5 py-1 rounded-lg border-2 text-[8px] font-bold transition-all shadow-sm uppercase tracking-wide ${formData.category ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : 'border-slate-100 bg-white text-slate-400 hover:border-emerald-500/30'}`}>
                        ...
                        {/* 5. In Loop */}
                        <div className="relative">
                            <button onClick={() => openModal('inLoop')} className={`flex items-center justify-center gap-1.5 px-2.5 py-1 rounded-lg border-2 text-[8px] font-bold transition-all shadow-sm uppercase tracking-wide ${formData.inLoopIds.length > 0 ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : 'border-slate-100 bg-white text-slate-400 hover:border-emerald-500/30'}`}>
                    ...
                    >
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="max-h-[50vh] overflow-y-auto px-1 py-1 flex flex-col gap-2 custom-scrollbar">
                                {filteredUsers.map(u => (
                                    <div key={u.userId} onClick={() => toggleUserSelect(u.userId, 'doerId')} className={`p-3.5 hover:bg-emerald-50/50 rounded-2xl cursor-pointer flex items-center gap-4 transition-all border-2 ${formData.doerId.includes(u.userId) ? 'bg-emerald-50 border-emerald-100 shadow-sm' : 'border-transparent'}`}>
                                        <div className="w-11 h-11 rounded-full bg-gradient-to-br from-emerald-400 to-emerald-500 flex items-center justify-center text-sm font-black text-white shrink-0 relative shadow-lg">
                                            ...
                                            {formData.doerId.includes(u.userId) && (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="text-[13px] font-black text-slate-800 truncate">{u.firstName} {u.lastName}</div>
                                            <div className="text-[11px] text-slate-400 font-bold truncate tracking-tight">{u.email}</div>
                                        </div>
                                    </div>
                                ))}
                                    ...
                                    <div className="flex flex-col gap-7">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-1.5 bg-slate-100/50 p-1.5 rounded-2xl border border-slate-100">
                                                {['Mobile App', 'WhatsApp', 'Email'].map(m => (
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                            <div className="flex items-center gap-4">
                                                <input
                                                    type="number"
                                                    min="1"
                                                    value={reminder.timingValue}
                                                    onChange={(e) => updateReminder(reminder.id, 'timingValue', e.target.value)}
                                                ...
                                                <div className="relative flex-1">
                                                    <select
                                                        value={reminder.timingUnit}
                                                        onChange={(e) => updateReminder(reminder.id, 'timingUnit', e.target.value)}
                                                    ...
                                                    <label key={relation} className="flex items-center gap-3 cursor-pointer group/opt">
                                                        <div className="relative flex items-center">
                                                            <input
                                                                ...
                                                                checked={reminder.timingRelation === relation}
                                                                onChange={() => updateReminder(reminder.id, 'timingRelation', relation)}
                                                            />
                                                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${reminder.timingRelation === relation ? 'bg-emerald-500 border-emerald-500 shadow-md shadow-emerald-500/20' : 'bg-white border-slate-200 peer-hover:border-emerald-500/30'}`}>
                                                                {reminder.timingRelation === relation && <div className="w-2.5 h-2.5 bg-white rounded-full"></div>}
                                                            </div>
                                                        </div>
                                                        <span className={`text-[11px] font-black uppercase tracking-widest transition-colors ${reminder.timingRelation === relation ? 'text-emerald-600' : 'text-slate-400 group-hover/opt:text-slate-600'}`}>{relation}</span>
                                                    </label>
                                                ))}
                                            </div>
                                        </div>
                                ...
                                {filteredLoopUsers.length === 0 ? renderEmptyState() : (
                                    <div className="grid grid-cols-1 gap-2.5">
                                        {filteredLoopUsers.map(u => (
                                            <div
                                                key={u.userId}
                                                onClick={() => toggleUserSelect(u.userId, 'inLoopIds')}
                                                className={`p-4 rounded-[28px] cursor-pointer flex items-center gap-5 transition-all border-2 group ${formData.inLoopIds.includes(u.userId) ? 'bg-emerald-50/50 border-emerald-500 shadow-sm shadow-emerald-500/10' : 'bg-white border-slate-50 hover:border-emerald-500/30'}`}
                                            >
                                                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-400 to-indigo-500 flex items-center justify-center text-sm font-black text-white shrink-0 relative shadow-lg">
                                                    {u.firstName.charAt(0)}{u.lastName.charAt(0)}
                                                    {formData.inLoopIds.includes(u.userId) && (
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="text-[16px] font-black text-slate-800 truncate tracking-tight">{u.firstName} {u.lastName}</div>
                                                    <div className="text-[12px] text-slate-400 font-bold truncate tracking-widest uppercase opacity-70">{u.email}</div>
                                                </div>
                                                {!formData.inLoopIds.includes(u.userId) && (
