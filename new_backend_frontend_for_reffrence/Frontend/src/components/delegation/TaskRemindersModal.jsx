...
const TaskRemindersModal = ({ isOpen, onClose, onSave, initialReminders = [] }) => {
    const [reminders, setReminders] = useState([
        { type: 'email', timeValue: 10, timeUnit: 'minutes', triggerType: 'before' }
    ]);
    ...
    useEffect(() => {
        if (initialReminders && initialReminders.length > 0) {
            ...
            setReminders([{ type: 'email', timeValue: 10, timeUnit: 'minutes', triggerType: 'before' }]);
        }
    ...
    const addReminder = () => {
        setReminders([...reminders, { type: 'email', timeValue: 10, timeUnit: 'minutes', triggerType: 'before' }]);
    };
            ...
            <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-200">
                                    ...
                                    {['before', 'after'].map((type) => (
                                        ...
                                        >
                                            <div className={`w-3 h-3 rounded-full border flex items-center justify-center ${
                                                reminder.triggerType === type ? 'border-emerald-500' : 'border-slate-300 dark:border-slate-700'
                                            }`}>
                                                {reminder.triggerType === type && <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />}
                                            </div>
                                            {type}
                                        </button>
                                    ))}
                ...
                <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2.5 bg-slate-50 dark:bg-slate-950/50">
                    ...
                    <button 
                        ...
                        className="flex-1.5 py-2 rounded-lg bg-emerald-500 text-white font-black uppercase tracking-widest text-[10px] shadow-lg shadow-emerald-500/10 hover:bg-emerald-600 transition-all flex items-center justify-center gap-2 active:scale-[0.98]"
                    >
                        <Save size={14} />
                        SAVE ALL
                    </button>
